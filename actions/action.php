...
if (!isset($_SESSION['etat'])) $_SESSION['etat'] = 'inconnu';

include 'Stats.php';
include 'Activite.php';

foreach (array('loginP','loginE','email') as $cle) {
    if (isset($_POST[$cle])) $_POST[$cle]=strtolower($_POST[$cle]);
}
    ...
    case 'ajoutP' :
	  ...
	  foreach ($phrases as $phrase) {
		  ...
		  if ($erreur == '') {
			...
			if ($erreur == '') ajoutP($_SESSION['loginP'], $_GET['login'],$phrase);
		  }
		  if ($erreur!='') $nbErreurs++;
	  }
...
switch ($_SESSION['etat']) {
  ...
  default :
    ...
    if (!isset($stats)) $stats = new Stats();
